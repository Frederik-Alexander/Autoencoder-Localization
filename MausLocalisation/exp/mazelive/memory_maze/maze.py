...
class MazeWithTargetsArena(mazes.MazeWithTargets):
    ...
    def regenerate(self, random_state):
        ...
        random_state.seed(123) # FA Diese zeile hinzugefügt 
...
class TextMazeVaryingWalls_OLD(labmaze.FixedMazeWithRandomGoals): # HABE ICH GEÄNDERT SIEHE OLD 
    """Augments standard generated labmaze with some walls marked with different chars."""
    def __init__(self, *args, **kwargs):
        self.fixed_layout = None  # Add a property to store the fixed layout
        ...
        #args = (_MAZE_2,num_spawns, _SPAWN_TOKEN,num_objects, _OBJECT_TOKEN,np.random.RandomState(234))
        ...
        #exit()
        #maze = super().__init__(entity_layer=_MAZE_1)
    


    def generate_fixed_layout(self):
        self.regenerate()  # Generate the maze layout once
        self.fixed_layout = np.copy(self.entity_layer)  # Store this layout

    def regenerate(self):
        #if self.already_generated == False:
        super().regenerate()
        self._block_variations()
        self.already_generated = True
            #self.fixed_layout = np.copy(self.entity_layer)
            #self.generate_fixed_layout()
...
class TextMazeVaryingWalls(labmaze.RandomMaze):
    """Augments standard generated labmaze with some walls marked with different chars."""
    def __init__(self, *args, **kwargs):
        print("INIT MAZE WITH SEED 123")
        kwargs['random_seed']= 123
        print("*args", args)
        print("**kwargs",kwargs) # **kwargs {'height': 11, 'width': 11, 'max_rooms': 6, 'room_min_size': 3, 'room_max_size': 5, 'max_variations': 26, 'spawns_per_room': 1, 'objects_per_room': 1, 'simplify': True, 'random_seed': 157374428}
